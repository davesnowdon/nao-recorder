...
class DryTranslator(object):
	...
	def detect_command(self, joint_dict):
		...
		# arms down?
		arms_down_args = {
			'both_command': 'arms.down',
			'left': {
				'command': 'arms.left_down',
				'pitch_joint':'LShoulderPitch',
				'pitch_max':90,
				'pitch_min':46,
				'pitch_offset_lambda': lambda x: round(90 - x),
		...
		arms_commands = arms_commands + self.eval_commands(joint_dict, arms_down_args)

		# arms back?
		arms_back_args = {
			'both_command': 'arms.back',
			'left': {
				'command': 'arms.left_back',
				'pitch_joint':'LShoulderPitch',
				'pitch_max':119.5,
				'pitch_min':91,
				'pitch_offset_lambda': lambda x: round(119.5 - x),
				'roll_joint':'LShoulderRoll',
				'roll_max':45,
				'roll_min':-90,
				'roll_offset_lambda': lambda x: round(-x)
		...
		arms_commands = arms_commands + self.eval_commands(joint_dict, arms_back_args)
